...
QUnit.test("Mention", function (assert) {
	var fixture = $('#qunit-fixture');

	fixture.html('<ul class="mentions"></ul>');

	_test_only_.Mention({ name: "Grant" });
	assert.equal($('.name', fixture).text(), "Grant", "name is injected correctly");
});